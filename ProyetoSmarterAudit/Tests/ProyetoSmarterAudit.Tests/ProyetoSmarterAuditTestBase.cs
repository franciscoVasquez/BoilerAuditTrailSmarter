...
{
    public abstract class ProyetoSmarterAuditTestBase : AbpIntegratedTestBase<ProyetoSmarterAuditTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected ProyetoSmarterAuditTestBase()
        {
        ...
        protected void UsingDbContext(Action<ProyetoSmarterAuditDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ProyetoSmarterAuditDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ProyetoSmarterAuditDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ProyetoSmarterAuditDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ProyetoSmarterAuditDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ProyetoSmarterAuditDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ProyetoSmarterAuditDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ProyetoSmarterAuditDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ProyetoSmarterAuditDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ProyetoSmarterAuditDbContext, Task<T>> func)
        {
            T result;
